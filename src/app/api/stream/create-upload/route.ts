...
function buildSignature(libraryId: string, apiKey: string, expires: number, videoId: string): string {
...
export async function POST(req: NextRequest) {
  ...
  // 1) Create the video object in Bunny (returns a GUID we use as UID/playbackId)
  ...
  const videoId =
    (createData && typeof createData === 'object' ? (createData as {guid?: string}).guid : null) ??
    null
  ...
  // 2) Precompute signature/expiry for TUS requests (client will use these headers)
